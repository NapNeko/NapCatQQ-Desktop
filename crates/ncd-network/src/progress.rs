//! 下载进度回调抽象不 import ncd-component 避免本 crate 反向依赖上层,调用方
...
/// 一次进度更新total None 表示服务端未给 Content-Length;speed_bps None 表示样本不足
#[derive(Debug, Clone)]
...
/// 下载进度接收端tick 由 download/race/chunked 节流后调用&self 非 &mut self,

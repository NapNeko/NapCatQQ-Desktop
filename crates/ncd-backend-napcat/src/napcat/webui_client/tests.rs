...
fn check_login_status_data_deserializes_legacy_field_names() {
    let json = r#"{"isLogin":true,"isOffline":false,"qrcodeurl":"data:image/png;base64,iVBOR"}"#;
...
fn check_login_status_data_uses_defaults_when_fields_missing() {
    ...
    let json = r#"{}"#;
    let data: CheckLoginStatusData = serde_json::from_str(json).expect("deserialize empty object");
    assert!(!data.is_login);
...
fn get_qq_login_info_data_defaults_online_to_unknown_when_missing() {
    let json = r#"{}"#;
    let data: GetQQLoginInfoData = serde_json::from_str(json).expect("deserialize empty object");

...
impl NapCatWebUiClient for StatusMockClient {
    async fn fetch_credential(&self, _port: u16, _token: &str) -> Result<String, NapCatWebUiError> {
...
async fn status_poll_two_signal_failures_publish_probe_unavailable_once() {
    ...
    assert_eq!(
        events
            .iter()
            .filter(|event| matches!(event, DomainEvent::NapCatLoginProbeUnavailable { .. }))
            .count(),
        1
    );
...
async fn apply_login_is_offline_keeps_session_without_invalidation() {
    ...
    assert!(
        events
            .iter()
            .any(|event| matches!(event, DomainEvent::NapCatLoginQrcodeRemoved { .. }))
    );
    assert!(
        !events
            .iter()
            .any(|event| matches!(event, DomainEvent::NapCatLoginInvalidated { .. }))
    );
}

...
//!
//! 当前仅承载 `napcat-webui-login` Spec 引入的 `WebUiPollerSettings`：
//! 控制 NapCat WebUI 登录态轮询间隔与离线通知开关。所有字段
//! 通过 ts-rs 派生导出到 `src-ui/core/ipc/generated/` 以避免前后端漂移。
...
mod tests {
    ...
    const LEGACY_CANONICAL_JSON: &str =
        r#"{"botLoginCheckInterval":5000,"botOfflineWebHookNotice":false,"botOfflineEmailNotice":false}"#;

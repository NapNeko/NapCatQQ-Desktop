...
//! ServerManager 会让 BotManager 绑死一种获取方式且难测抽成 trait 后:
...
/// 把 RuntimeTarget 解析成可用的 host失败返回人话错误(连不上远端 / server_id
/// 不存在等),由 BotManager 转成启动失败事件
#[async_trait]
pub trait HostResolver: Send + Sync {
    /// 解析 target -> hostLocal 给本机 host;Server(id) 连远端
...
/// 始终返回固定本机 host 的 resolver用于:还没接入远端的过渡期,纯本机
/// 部署场景,以及测试Server(id) 也返回本机 host(过渡期不报错,行为等同

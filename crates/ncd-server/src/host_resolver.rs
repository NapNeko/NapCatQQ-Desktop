...
//!
//! 放 ncd-server(与 ServerManager 同轴),src-tauri 实装 trait 注入 BotManager。
...
/// Host 解析失败（连不上远端 / server_id 不存在等）
...
impl HostResolveError {
    pub fn message(msg: impl Into<String>) -> Self {
        Self::Message(msg.into())
    }
}

impl From<String> for HostResolveError {
    fn from(value: String) -> Self {
        Self::Message(value)
    }
}

impl From<&str> for HostResolveError {
    fn from(value: &str) -> Self {
        Self::Message(value.to_string())
    }
}

/// 把 RuntimeTarget 解析成可用的 host。失败由 BotManager 转成启动失败事件。
#[async_trait]
...
impl HostResolver for LocalOnlyHostResolver {
    async fn resolve(&self, _target: &RuntimeTarget) -> Result<Arc<dyn Host>, HostResolveError> {

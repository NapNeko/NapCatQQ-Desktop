...
async fn spawn_stdout_reader_forwards_cleaned_line() {
    ...
    let event_bus = Arc::new(BroadcastEventBus::default());
    let mut bus_sub = event_bus.subscribe(EventFilter::kind(DomainEventKind::SnowLumaDaemonLog));
...
/// 多 Bot 共享:ref_count>1 时 release 只减计数,不关 daemon
#[tokio::test]
async fn daemon_release_keeps_ready_while_refs_remain() {
    let daemon = build_smoke_daemon();
    daemon.test_force_ready_with_ref_count(2).await;
...
/// 最后一个引用 release 后必须 shutdown:Stopped + ref_count==0
/// 对应 issue:有头模式停用 Bot 后 node.exe 残留导致重启多开
#[tokio::test]
async fn daemon_release_last_ref_shuts_down() {
    let daemon = build_smoke_daemon();
    daemon.test_force_ready_with_ref_count(1).await;
...
#[tokio::test]
async fn daemon_release_last_ref_blocks_concurrent_ensure_running() {
    let daemon = build_smoke_daemon();
    daemon.test_force_ready_with_ref_count(1).await;

    daemon.release().await;
    let result = daemon.ensure_running(Duration::from_millis(50)).await;

    // 已 Stopped:会走 starter;无 node.exe 时 starter 失败回滚,绝不能 Ok 复用旧 Ready
    assert!(result.is_err(), "must not reuse daemon after last release");
    assert_eq!(daemon.state().await, DaemonState::Stopped);
}

...
fn render_daemon_globals_uses_override_when_present() {
    ...
    let returned =
        render_daemon_globals(snow, runtime, Some(override_pwd), 5099).expect("render override");
...
#[test]
fn parse_bound_webui_port_from_logs_prefers_using_instead() {
    let lines = [
        "20:06:56 INFO [WebUI] listening http://0.0.0.0:5099",
        "20:06:57 WARN [WebUI] port 5099 is in use, using 5103 instead",
        "20:06:58 INFO [WebUI] listening http://0.0.0.0:5103",
    ];
    assert_eq!(parse_bound_webui_port_from_logs(lines), Some(5103));
}

#[test]
fn parse_bound_webui_port_from_logs_falls_back_to_listening() {
    let lines = [
        "noise",
        "20:06:56 INFO [WebUI] listening https://127.0.0.1:6111",
    ];
    assert_eq!(parse_bound_webui_port_from_logs(lines), Some(6111));
}

#[test]
fn parse_bound_webui_port_from_logs_returns_none_when_absent() {
    let lines = ["spawn ok", "login failed"];
    assert_eq!(parse_bound_webui_port_from_logs(lines), None);
}

#[test]
fn parse_bound_webui_port_from_logs_handles_https_and_ipv6_style() {
    let lines = ["INFO [WebUI] listening https://[::1]:6123"];
    assert_eq!(parse_bound_webui_port_from_logs(lines), Some(6123));
}

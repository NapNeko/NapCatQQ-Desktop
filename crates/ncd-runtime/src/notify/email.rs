...
pub fn send_test_email(settings: &OfflineEmailSettings) -> Result<(), String> {
    validate_settings(settings)?;
    let subject = "测试 NapCatQQ-Desktop 发件功能";
    let body = format!(
        "<html><body><p>这是一封测试邮件。</p><p>时间: {}</p></body></html>",
...
<p>状态: {event}</p>
<p>时间: {at}</p>
<p>本邮件由 NapCatQQ Desktop 自动发送。</p>
</body></html>"#,
        name = name,

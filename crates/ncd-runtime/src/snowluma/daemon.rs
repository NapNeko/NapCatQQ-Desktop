...
/// stdout 广播容量。订阅滞后会被 broadcast 通道 overwrite。
...
impl SnowLumaDaemon {
    ...
    /// 返回 + 进入 Crashed/Stopped 转换。当 child 不在 inner 时是 no-op。
    ...
    async fn wait_for_ready(
        ...
        loop {
            ...
            // 等 ready_notify 会一直等到超时。提前快照可以短路。
            {
...
async fn watch_exit(daemon: Weak<SnowLumaDaemon>) {
    ...
    // 阻塞等子进程退出。child.wait() 不需要 mutable lock，节省锁占用。

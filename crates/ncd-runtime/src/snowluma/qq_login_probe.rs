...
//!
//! 给定一个 qq_id，在本机找到正登录该账号的 QQ.exe 主进程 PID。两级探测：
//!
...
///    做深链接兜底。深链接只打该 PID 自己的端口，不再无差别全段盲扫。
///
...
async fn probe_pid(pid: u32, total_qq_count: usize) -> Option<ProbedQqLogin> {
    ...
    if ports.is_empty() {
        ...
        // 映射。低于阈值当作"唯一实例等待登录"放弃；超阈值环境复杂也不盲扫，

...
import { validateNoneBot2Config } from '../../../core/domain/apps/nonebot2Config';
import type {
    AppConfigError,
    AppConfigIssue,
...
export function useNoneBot2ConfigForm(instanceId: string, enabled: boolean, instanceName: string) {
    const remote = useAppInstanceConfig(instanceId, enabled);
    const [form, setFormState] = useState<NoneBot2InstanceConfig | null>(null);
    const [pristine, setPristine] = useState<NoneBot2InstanceConfig | null>(null);
    ...
    useEffect(() => {
        ...
        if (!env || env.config.framework !== 'nonebot2') return;
    ...
    const setForm = useCallback((next: NoneBot2InstanceConfig) => {
        setFormState(next);
        setServerIssues((prev) => (prev.length ? [] : prev));
    }, []);

    const clientIssues = useMemo(() => (form ? validateNoneBot2Config(form) : []), [form]);

...
当前处于：**v1→v2 方向切换 + 代码清理（进行中）**

### 总体判断

- v1 的 SSH 基础设施（SSH 客户端、SFTP、错误模型、部署骨架）**可复用**
- v1 的 Agent/Daemon 全套代码（Go Daemon、JSON-RPC、WebSocket 客户端、Agent UI）**需归档**
- v2 的 P0（操作抽象层 + 服务器管理）尚未开始
...
- [ ] 实现 LocalBackend（封装现有本地逻辑）
- [ ] 实现 RemoteBackend 骨架（SSH/SFTP）
- [ ] 服务器配置模型和持久化
- [ ] SSHClient 升级为持久连接管理器（keepalive、重连、多 channel）
- [ ] 服务器管理 UI（添加/测试/删除）

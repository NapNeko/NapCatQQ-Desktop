...
- **一键远程部署**：通过 SSH 把 NapCat 安装到主流 Linux 服务器（如果有问题请提交issue），自动处理依赖、目录、服务脚本
- **自动接管**：已经部署了 NapCat 的服务器会自动扫描
- **多服务器管理**：添加/编辑/连通性测试/批量操作，统一面板管理多台服务器
- **远程 Bot 运行**：在远端跑 Bot，本地端口自动转发，使用体验和本地 Bot 一致(就是本地也能直接打开服务器的webui，走的ssh隧道)
...
- **持久数据迁移**：`MigrationDialog` 支持将本地 Bot 配置/数据迁移到远端服务器(待完善)

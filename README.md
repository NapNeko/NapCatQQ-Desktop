...
- **WIP：组件库与页面基础层整理**
- 近期会继续统一 UI 组件的基础样式和卡片表面实现，减少重复逻辑，稳住整体视觉一致性。
- 同时会继续完善日志页、高亮器和版本检测链路，让安装完成后的刷新、日志展示和版本识别更可靠。
- 这些工作主要是在为后续版本迭代做铺垫，先把桌面端的基础能力和交互稳定下来。

### 我正在做什么

- 整理和收敛重复的 UI 基类，统一卡片、设置项和页面容器的实现方式。
- 优化日志查看体验，包括日志清洗、语法高亮和展示字号。
- 修正版本检测与刷新时序，保证安装、更新和按钮状态切换更准确。

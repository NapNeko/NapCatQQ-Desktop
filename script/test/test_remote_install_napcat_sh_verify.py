# -*- coding: utf-8 -*-
"""脚本级单测: ``verify_napcat_archive_sha256`` (P5 安全收尾 F1.4).

直接用 bash subprocess 跑 ``remote_install_napcat.sh`` 中 SHA256 校验函数, 验证:

- 期望 hash 一致 -> exit 0
- 期望 hash 不一致 -> exit 36, archive 被删
- 缺失 ``$NAPCAT_EXPECTED_SHA256`` -> exit 0 (warn skip, 兼容老客户端)
- ``sha256sum`` 缺失但 ``openssl`` 可用 -> exit 0
...
def _find_bash() -> str | None:
    ...
    ``sha256sum '<windows-path>'`` 拿不到文件. 优先选 Git Bash / MSYS2 /
...
def _extract_verify_function() -> str:
    """从 install 脚本中切出 ``verify_napcat_archive_sha256`` 函数体.

    按 ``verify_napcat_archive_sha256()`` 起 / 第一个独立 ``}`` 终止 (函数末尾)
    切片. 该函数自身无嵌套 ``}``, 切片足够稳健.
    """
    text = SCRIPT_PATH.read_text(encoding="utf-8")
    start = text.find("verify_napcat_archive_sha256()")
    assert start != -1, "未在脚本中找到 verify_napcat_archive_sha256 函数定义"
    end = text.find("\n}\n", start)
    assert end != -1, "未在脚本中找到 verify_napcat_archive_sha256 函数闭合"
...
def _run_verify(
    *,
    ...
    extra_setup: str = "",
    sha256sum_available: bool = True,
    openssl_available: bool = True,
    tmp_path: Path,
) -> tuple[int, str, bool]:
    """运行 ``verify_napcat_archive_sha256`` 并返回 ``(exit_code, output, archive_still_exists)``."""
    archive = tmp_path / "NapCat.Shell.zip"
    archive.write_bytes(archive_content)

    func_body = _extract_verify_function()

    # 准备 PATH 屏蔽: 若指定 sha256sum/openssl 不可用, 把 bash 内的 ``command -v``
    # 改成永远返回非零. 用 ``command()`` shell builtin override 实现.
    overrides: list[str] = []
    if not sha256sum_available:
    ...
    if expected_env is not None:
        env_set = f'export NAPCAT_EXPECTED_SHA256={shell_quote(expected_env)}\n'
    ...
    script = (
        ...
        + func_body
        + "\n"
        + f'verify_napcat_archive_sha256 {shell_quote(str(archive))}\n'
    )
...
def test_verify_passes_when_hash_matches(tmp_path: Path) -> None:
    ...
    assert "sha256 verified ok" in output
...
def test_verify_skips_when_env_missing(tmp_path: Path) -> None:
    """``NAPCAT_EXPECTED_SHA256`` 未设置 -> warn skip, exit 0, archive 保留."""
    code, output, archive_kept = _run_verify(
...
def test_verify_uses_openssl_when_sha256sum_missing(tmp_path: Path) -> None:
    """没有 ``sha256sum`` 时退化到 ``openssl dgst -sha256``."""
...
def test_verify_fails_when_no_hash_tool_available(tmp_path: Path) -> None:
    """sha256sum 与 openssl 都不可用 + 要求校验 -> exit 36, archive 被删."""
    code, output, archive_kept = _run_verify(
        archive_content=b"any content",
        expected_env="aa" * 32,
        sha256sum_available=False,
    ...
    assert "neither sha256sum nor openssl" in output

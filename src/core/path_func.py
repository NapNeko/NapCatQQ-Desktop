    ...
    def get_qq_path() -> Path:
        ...
        try:
            if Path(cfg.get(cfg.qq_path)) == Path.cwd():
                ...
                path = Path(winreg.QueryValueEx(key, "UninstallString")[0]).parent
                cfg.set(item=cfg.qq_path, value=path, save=True)
                return path
            else:
                return True, cfg.get(cfg.qq_path)

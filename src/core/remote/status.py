...
class RemoteRuntimeService:
    ...
    def get_status_for_bot(self, qq_id: str) -> RemoteNapCatStatus:
        """读取指定 Bot 的运行状态(P2 多 Bot 版).

        相比 [`get_status`](src/core/remote/status.py) 的"任意一个 Bot"语义,
        该方法严格按 ``qq_id`` 过滤进程, 同时优先消费 launcher 脚本写出的
        ``status_<qq_id>.json``.
        ...
        Returns:
            该 Bot 的运行状态; ``running=False`` 时 ``pid`` 必为 None.
        """
        # 1. 通过 pgrep + cmdline 二次确认匹配 -q <qq_id>
        pgrep_result = self.backend.run(
            f"pgrep -f 'qq --no-sandbox -q {qq_id}' 2>/dev/null || true"
        )
        pid: int | None = None
        running = False
        # 真实 cmdline 形如 ``/usr/local/bin/qq --no-sandbox -q 2707600964`` 或
        # ``/bin/sh /usr/bin/xvfb-run -a /usr/local/bin/qq --no-sandbox -q 2707600964``,
        # 即 ``-q`` 前是空白. ``\b`` 在 "空白 + -" 处不会触发, 故必须用
        # ``(?:^|\s)`` 锚定; 末尾用 ``(?!\d)`` 防止 qq_id 误匹配为更长数字的前缀.
        cmdline_pattern = re.compile(
            rf"(?:^|\s)-q\s+{re.escape(qq_id)}(?!\d)"
        )
        if pgrep_result.ok and pgrep_result.stdout.strip():
            for raw in pgrep_result.stdout.strip().splitlines():
                raw = raw.strip()
                if not raw.isdigit():
                    continue
                candidate_pid = int(raw)
                cmdline = self.backend.run(
                    f"ps -o cmd= -p {candidate_pid} 2>/dev/null || true"
                )
                if cmdline.ok and cmdline_pattern.search(cmdline.stdout):
                    pid = candidate_pid
                    running = True
                    break
        ...
        status_result = self.backend.run(
            f'test -f "{status_path}" && cat "{status_path}" || true'
        )
        payload = self._parse_status_payload(status_result.stdout)

        # 3. 版本号: 复用 napcat.mjs 探测, 与 get_status 一致
        version = self._as_string(payload.get("version"))
        if not version:
            version = self._detect_napcat_version_from_mjs()

        # log_file 优先取 status payload 中的, 否则走默认 napcat_<qq_id>.log
        log_file = self._as_string(payload.get("log_file")) or (
            f"{self.paths.log_dir}/napcat_{qq_id}.log"
        )

        return RemoteNapCatStatus(
            running=running,
            pid=pid,
            qq=qq_id if running else self._as_string(payload.get("qq")),
            version=version,
            log_file=log_file,
            raw_payload=payload,
        )
    ...
    def get_status(self) -> RemoteNapCatStatus:
        ...
        .. note::
            该方法仍按"任意一个匹配的 NapCat 进程"返回, 主要用于
            部署器与单服务器探测场景 (P0/P1). 多 Bot 场景请使用
            [`get_status_for_bot`](src/core/remote/status.py).
        """
    ...
    def _detect_napcat_version_from_mjs(self) -> str | None:
        """从 ``napcat.mjs`` 抓出 ``napCatVersion`` 字段; 探测失败时返回 None.

        与 [`LinuxCoreDeployment._detect_napcat_version`](src/core/remote/deployment.py)
        以及 [`VersioningService._get_napcat_version_from_mjs`](src/core/versioning/service.py)
        逻辑保持完全一致.
        """

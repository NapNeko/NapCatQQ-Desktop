...
class ServerManager(QObject):
    ...
    def _lookup_napcat_expected_sha256(self) -> str | None:
        """同步获取远端 latest NapCat 的期望 SHA256.

        流程: 拉取上游 release (带缓存) -> 拉取 NapCat 远端 latest 版本号 ->
        ...
        try:
            ...
            if entry is None:
                logger.warning(
                    f"NapCat 完整性校验跳过: 上游 release 中没有版本 {version}",
                    LogType.NETWORK,
                    LogSource.CORE,
                )
                return None
            logger.info(
                f"NapCat 完整性校验已启用: version={entry.version}, sha256_prefix={entry.shell_sha256[:16]}...",
                LogType.NETWORK,
                LogSource.CORE,
            )
            return entry.shell_sha256
    ...
    def deploy_server(
        ...
        *,
        ...
        try:
            ...
            # P5 F1.4: 在调用 install_napcat 之前先查询上游 SHA256;
            # 取不到时静默跳过 (远端脚本端 fallback 为 warn skip), 不阻断部署
            # - 远端 worker 线程不便弹用户对话框, 这里走"网络不稳时允许继续"策略.
            expected_sha256 = self._lookup_napcat_expected_sha256()
            ...
            try:
                ...
                if self.is_cancel_requested(server_id):
                    raise RemoteDeploymentCancelledError(cause=exc) from exc
                # P5 F1.4: 远端脚本退出码 36 -> SHA256 不匹配, 单独 stage 标签便于
                # friendly_errors 与 UI 提示走"完整性校验"专属文案
                stage_label = "install_napcat"
                stderr_text = getattr(exc, "stderr", "") or ""
                exit_status = getattr(exc, "exit_status", None)
                if exit_status == 36 or "sha256 mismatch" in stderr_text.lower():

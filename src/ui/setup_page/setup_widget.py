...
class SetupWidget(ScrollArea):
    ...
    def initialize(self, parent) -> "SetupWidget":
        """
        初始化
        """
        # 背景 QPixmap
    ...
    def connectSignal(self):
        ...
        """
        # 连接重启提示
        cfg.appRestartSig.connect(self.show_RestartTooltip)

        # 连接个性化相关
        cfg.themeChanged.connect(
            lambda theme: setTheme(cfg.get(theme))
        )
        self.theme_color_card.colorChanged.connect(setThemeColor)

    def show_RestartTooltip(self):
        """
        显示重启提示
        """
        InfoBar.success(

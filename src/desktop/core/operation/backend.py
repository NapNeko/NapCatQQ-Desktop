# -*- coding: utf-8 -*-
"""[`OperationBackend`](src/desktop/core/operation/backend.py) 接口与共享数据模型。
...
- 进度回调 [`ProgressCallback`](src/desktop/core/operation/backend.py) 用于安装/部署等长耗时操作向 UI 反馈进度
"""
...
class FileEntry:
    """目录条目, 由 [`OperationBackend.list_dir`](src/desktop/core/operation/backend.py) 返回。"""
...
@dataclass(slots=True)
class ProcessStatus:
    """NapCat 进程状态, 由 [`OperationBackend.get_process_status`](src/desktop/core/operation/backend.py) 返回。
...
class OperationBackend(ABC):
    ...
    通过 [`LocalBackend`](src/desktop/core/operation/local_backend.py)
    或 [`RemoteBackend`](src/desktop/core/operation/remote_backend.py)
    在本地或远端透明执行。
    """
    ...
    def start_napcat(self, qq_id: str, config: "Config") -> ProcessStatus:
        ...
        - RemoteBackend: 通过 SSH 执行远端启动脚本(参考 [`templates.build_linux_deploy_script`](src/desktop/core/remote/templates.py))
        """

...
# (src/desktop/core/versioning/service.py) 完全一致.
#
...
class LinuxCoreDeployment:
    ...
    P1 之后建议使用 [`install_linuxqq`](src/desktop/core/remote/deployment.py)
    与 [`install_napcat`](src/desktop/core/remote/deployment.py) 分两步部署，
    ...
    def _detect_napcat_version(self) -> str | None:
        ...
        (src/desktop/core/versioning/service.py) **保持完全一致** 的解析逻辑:
    ...
    def _run_script_with_progress(
        ...
        非 [`SSHClient`](src/desktop/core/remote/ssh_client.py) 的执行后端不支持流式读取,

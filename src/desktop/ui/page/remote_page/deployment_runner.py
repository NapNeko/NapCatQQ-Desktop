# -*- coding: utf-8 -*-
"""[`DeploymentRunner`](src/desktop/ui/page/remote_page/deployment_runner.py): 后台执行远端部署。

仿 [`ConnectionTester`](src/desktop/ui/page/remote_page/connection_tester.py) 的模式,
通过 [`QRunnable`](https://doc.qt.io/qt-6/qrunnable.html) 把 [`ServerManager.deploy_server`](src/desktop/core/remote/server_manager.py)
推到 [`QThreadPool`](https://doc.qt.io/qt-6/qthreadpool.html) 里执行。

注意: 本 runner 仅负责调度, **不**重复 emit `progress` / `finished` 信号:
- 进度实时性: 由 [`ServerManager.deployment_progress`](src/desktop/core/remote/server_manager.py) 直接 emit, UI 端订阅该信号即可
...
class DeploymentRunnerSignals(QObject):
    """[`DeploymentRunner`](src/desktop/ui/page/remote_page/deployment_runner.py) 信号载体。"""

    finished = Signal(str)  # (server_id)


class DeploymentRunner(QRunnable):
    """后台执行 [`ServerManager.deploy_server`](src/desktop/core/remote/server_manager.py)。
    ...
    def run(self) -> None:  # noqa: D401 - 实现 QRunnable.run
        ...
        from src.desktop.core.remote import ServerManager
...
class RedetectRunnerSignals(QObject):
    """[`RedetectRunner`](src/desktop/ui/page/remote_page/deployment_runner.py) 信号载体。"""
...
class RedetectRunner(QRunnable):
    """后台执行 [`ServerManager.redetect_versions`](src/desktop/core/remote/server_manager.py)。

    相比 [`DeploymentRunner`](src/desktop/ui/page/remote_page/deployment_runner.py) 仅探测版本,
...
class RollbackRunnerSignals(QObject):
    """[`RollbackRunner`](src/desktop/ui/page/remote_page/deployment_runner.py) 信号载体。"""
...
class RollbackRunner(QRunnable):
    """后台执行 [`ServerManager.rollback_server`](src/desktop/core/remote/server_manager.py)。
